<script setup lang="ts">
  ...
  type GalleryPreviewSlot =
    | { type: 'image'; image: GalleryImageWithFile; originalIndex: number }
  ...
  const getDeletedImagesConflictWarning = async (): Promise<string | null> => {
    ...
    const missingImageCount = images.value.filter(
      image => !image.isNew && !image.id.startsWith('pending-') && !serverImageIds.has(image.id)
    ).length;
...
<template>
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div
        class="relative lg:col-span-8 lg:order-1 lg:self-start bg-[rgb(60,0,0)] rounded-xl border border-[#c9c9c9]/20 flex flex-col overflow-hidden"
      >
