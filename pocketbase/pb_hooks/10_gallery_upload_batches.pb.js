...
routerAdd('POST', '/api/shifu/upload-batches/{batchId}/cancel', (e) => {
	...
	if (targetType === 'gallery') {
		try {
			while (true) {
				const records = e.app.findRecordsByFilter('gallery_images', 'uploadBatchId = {:batchId}', '-created', 500, 0, { batchId: batch.id });
				if (!records.length) {
					break;
				}
			...
			if (targetId && targetId !== 'new' && snapshotCoverName) {
				...
				if (snapshotCoverName === '__none__') {
					...
					try {
						...
						album.set('cover', snapshotFilesystem.getReuploadableFile(batch.baseFilesPath() + '/' + snapshotFileName, true));
						e.app.save(album);
					} finally {
...
routerAdd('POST', '/api/shifu/upload-batches/{batchId}/complete', (e) => {
	...
	if (String(batch.get('targetType') || '') === 'album') {
		...
		const snapshotFileName = Array.isArray(snapshotFileValue) ? String(snapshotFileValue[0] || '') : String(snapshotFileValue || '');
...
routerAdd('POST', '/api/shifu/upload-batches/{batchId}/album-cover', (e) => {
	...
	try {
		if (!snapshotCoverName) {
			...
			if (currentCoverName) {
				snapshotFilesystem = e.app.newFilesystem();
				batch.set('snapshotFile', snapshotFilesystem.getReuploadableFile(album.baseFilesPath() + '/' + currentCoverName, true));
		...
		try {
			...
			if (latestSnapshotCoverName === '__none__') {
				...
				try {
					...
					latestAlbum.set('cover', latestSnapshotFilesystem.getReuploadableFile(latestBatch.baseFilesPath() + '/' + latestSnapshotFileName, true));
					e.app.save(latestAlbum);
				} finally {
...
cronAdd('gallery-upload-batch-cancel-cleanup', '*/10 * * * *', () => {
	...
	const batches = $app.findRecordsByFilter(uploadBatchesCollection, 'status = {:cancelling} || status = {:cancelled}', '-updated', 500, 0, {
	...
	for (const batch of batches) {
		try {
			...
			if (targetType === 'gallery') {
				while (true) {
					const records = $app.findRecordsByFilter('gallery_images', 'uploadBatchId = {:batchId}', '-created', 500, 0, { batchId: batch.id });
					if (!records.length) {
						break;
					}
