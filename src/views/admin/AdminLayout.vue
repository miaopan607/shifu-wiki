<script setup lang="ts">
  ...
  const uploadTaskCount = computed(
    () => uploadStore.tasks.value.filter(task => task.status === 'uploading' || task.status === 'paused').length
  );
...
<template>
  <div class="admin-layout h-screen flex flex-col overflow-hidden bg-[rgb(77,0,0)] font-serif">
    <header
      class="h-14 bg-[rgb(60,0,0)] border-b border-[#c9c9c9]/20 flex items-center justify-between px-4 md:px-6 z-50 shrink-0"
    >
      ...
      <div class="flex items-center gap-3">
        ...
        >
          ...
          <span
            ...
            class="inline-flex min-w-5 h-5 items-center justify-center rounded-full bg-red-300 text-[rgb(77,0,0)] text-xs font-semibold px-1"
          >
            {{ uploadTaskCount }}
          </span>
        </button>
