<script setup lang="ts">
  ...
  const handleLogin = async () => {
    ...
    try {
      await pb.collection('users').authWithPassword(username.value.trim(), password.value);
    ...
    <div class="w-full max-w-md">
      <div class="bg-[rgb(60,0,0)] rounded-xl border border-[#c9c9c9]/20 p-8">
        ...
        <form class="space-y-5" @submit.prevent="handleLogin">
          <div>
            <label for="username" class="block text-sm text-[#c9c9c9] mb-1.5"> 用户名 </label>
            <input
              id="username"
              v-model="username"
              type="text"
          ...
          <div>
            <label for="password" class="block text-sm text-[#c9c9c9] mb-1.5"> 密码 </label>
            <div class="relative">
              <input
                id="password"
                v-model="password"
