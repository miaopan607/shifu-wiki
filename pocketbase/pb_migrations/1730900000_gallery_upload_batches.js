migrate(
	(app) => {
		...
		try {
			galleryImages.addIndex('idx_gallery_images_upload_identity', true, 'uploadBatchId, clientUploadId', "uploadBatchId != '' AND clientUploadId != ''");
		} catch (_) {}
		...
		const uploadBatches = new Collection({
			...
			indexes: ['CREATE INDEX idx_upload_batches_owner_status ON upload_batches (ownerId, status)', 'CREATE INDEX idx_upload_batches_target ON upload_batches (targetType, targetId)'],
		});
