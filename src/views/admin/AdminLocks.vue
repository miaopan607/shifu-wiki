...
const buildTargetTitleMap = async (editLocks: EditLockRecord[]): Promise<Map<string, string>> => {
	...
	const entries = await Promise.all(
		uniqueTargets.map(async (lock) => [getTargetKey(lock.collection, lock.recordId), await fetchTargetTitle(lock.collection, lock.recordId)] as const),
	);
...
const allFilteredSelected = computed(() => filteredLockIds.value.length > 0 && filteredLockIds.value.every((id) => selectedLockIdSet.value.has(id)));
const someFilteredSelected = computed(() => filteredLockIds.value.some((id) => selectedLockIdSet.value.has(id)) && !allFilteredSelected.value);
...
<template>
	<div class="relative min-h-100">
		...
		<div v-else class="relative space-y-6">
				...
				<table class="w-full text-left">
					...
					<tbody class="divide-y divide-[#c9c9c9]/10">
						...
						>
							<td class="px-4 py-3">
								<div class="flex items-center justify-center">
									<input type="checkbox" class="lock-checkbox" :checked="selectedLockIdSet.has(lock.id)" @click.stop @change="toggleLockSelection(lock.id)" />
								</div>
							</td>
							<td class="px-4 py-3">
							...
							<td class="px-4 py-3">
								<div class="flex items-center justify-end gap-2">
									<button v-if="lock.targetPath" @click.stop="openTargetRecord(lock)" class="px-3 py-1.5 text-sm text-[#c9c9c9] hover:bg-white/5 rounded-lg transition-colors">
										打开记录
									</button>
									<button @click.stop="confirmDelete(lock.id)" class="px-3 py-1.5 text-sm text-red-300 hover:bg-red-500/10 rounded-lg transition-colors">移除锁</button>
								</div>
							</td>
						</tr>
					</tbody>
				...
				<div class="bg-[rgb(60,0,0)] border border-[#c9c9c9]/20 rounded-xl max-w-sm w-full p-6 shadow-2xl">
					...
					<div class="flex items-center justify-between gap-3">
						<label class="inline-flex items-center gap-2 cursor-pointer select-none text-sm text-[#c9c9c9]">
							<input v-model="skipSingleDeleteConfirmDraft" type="checkbox" class="lock-checkbox shrink-0" @click.stop />
							<span>不再提示</span>
						</label>
						<div class="flex justify-end gap-3">
							<button @click="cancelDelete" class="px-4 py-2 text-[#c9c9c9] hover:bg-white/5 rounded-lg transition-colors" :disabled="deleting">取消</button>
							<button
			...
			<div v-if="batchDeleteConfirm" class="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
				<div class="bg-[rgb(60,0,0)] border border-[#c9c9c9]/20 rounded-xl max-w-md w-full p-6 shadow-2xl">
					...
					<div class="flex justify-end gap-3">
						<button @click="cancelBatchDelete" class="px-4 py-2 text-[#c9c9c9] hover:bg-white/5 rounded-lg transition-colors" :disabled="batchDeleting">取消</button>
						<button @click="deleteSelectedLocks" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors flex items-center gap-2" :disabled="batchDeleting">
							<span v-if="batchDeleting" class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
							确认批量移除
						</button>
					</div>
				</div>
