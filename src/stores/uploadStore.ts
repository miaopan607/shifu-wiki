...
import type { BatchUploadTask, CreateBatchTaskParams, FileUploadInfo, FileUploadStatus, GlobalUploadPanelState, UploadStats } from '@/types/upload';
import { createBatchUploadTask, isTaskActive, isTaskCompleted, uploadFileWithXHR, updateTaskStats } from '@/lib/uploadManager';
...
const hasBlockingTasks = computed(() => tasks.value.some((task) => task.status === 'pending' || task.status === 'uploading' || task.status === 'paused'));
...
function refreshSchedulerState(): void {
	const nextQueuedTask = tasks.value.find((task) => task.status === 'uploading' && task.files.some((file) => file.status === 'pending'));
...
function appendFilesToTask(taskId: string, params: Pick<CreateBatchTaskParams, 'files' | 'sorts' | 'clientIds'>): FileUploadInfo[] {
	const task = getTask(taskId);
	if (!task) return [];
...
function replaceTaskFiles(taskId: string, params: Pick<CreateBatchTaskParams, 'files' | 'sorts' | 'clientIds'>): FileUploadInfo[] {
	const task = getTask(taskId);
	if (!task) return [];
...
function startPendingTasks(targetId: string, targetType: 'gallery' | 'album'): void {
	tasks.value.forEach((task) => {
		if (task.targetType === targetType && (task.targetId === targetId || task.targetId === 'new') && task.status === 'pending') {
...
function updateFileStatus(taskId: string, fileId: string, status: FileUploadStatus, updates?: Partial<FileUploadInfo>): void {
	const task = getTask(taskId);
	if (!task) return;
...
function abortActiveRequests(taskId: string): void {
	for (const [fileId, request] of activeRequests.entries()) {
...
async function rollbackCancelledTaskUploads(task: BatchUploadTask): Promise<void> {
	...
	if (task.type === 'gallery_images') {
		const uploadedIds = new Set(task.files.map((file) => file.uploadedRecordId).filter((id): id is string => Boolean(id)));
		...
		if (uploadedIds.size > 0) {
			await Promise.allSettled(Array.from(uploadedIds).map((recordId) => pb.collection('gallery_images').delete(recordId)));
		}
	}
...
async function processQueue(): Promise<void> {
	...
	while (activeRequests.size < maxConcurrentUploads.value) {
		const nextTask = tasks.value.find((task) => task.status === 'uploading' && task.files.some((file) => file.status === 'pending'));
		if (!nextTask) {
			refreshSchedulerState();
			return;
		}
...
async function uploadSingleFile(task: BatchUploadTask, fileInfo: FileUploadInfo): Promise<void> {
	...
	try {
		...
		const method = 'POST';
		const url = task.type === 'gallery_images' ? `${pb.baseUrl}/api/collections/gallery_images/records` : `${pb.baseUrl}/api/shifu/upload-batches/${uploadBatchId}/album-cover`;
		...
		updateFileStatus(task.id, fileInfo.id, 'success', {
			progress: 100,
			uploadedRecordId: task.type === 'gallery_images' && typeof response === 'object' && response && 'id' in response ? String((response as { id: string }).id) : undefined,
		});
	} catch (error) {
		activeRequests.delete(fileInfo.id);
		const latestTask = getTask(task.id) ?? task;
